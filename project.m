...
desired_end_time = 60;    % Replace with your desired end time

% Convert times to sample indices
desired_start_sample = round(desired_start_time * Fs);
desired_end_sample = round(desired_end_time * Fs);

% Set parameters based on desired segment
t1 = desired_start_sample;
t2 = desired_end_sample;

% analyze the desired window of the song
y = song(t1:t2);
[~,n] = size(y);
t = linspace(t1, t2, n);
if ~mute
...
audiowrite('happy_birthday_window.wav', y, Fs);
...
letter
audiowrite('happy_birthday_recreated.wav',recreate_song,fs);
